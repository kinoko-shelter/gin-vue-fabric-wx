...
const myAmapFun = new amapFile.AMapWX({ key: '8486d21c23c54a65a83a9bea7ab33d74' });
...
Page({
  ...
  onLoad: function (options) {
    ...
    var sendData = Object.assign({}, options);

    var startLat = options.startPoint.split(",")[1];
    var startLon = options.startPoint.split(",")[0];
    var endLat = options.endPoint.split(",")[1];
    var endLon = options.endPoint.split(",")[0];
    ...
    Promise.all([
      ...
      const [startName, endName] = results.map(decodeURIComponent);
      const start_info = "当前位置:" + startName
      const end_info = "充电桩:" + endName
      // 确保sendData使用的是解码后的名称
      options.startName = start_info;
      options.endName = end_info;
      // 设置页面数据
      that.setData({
        sendData: options,
        markers: [{
    ...
    // 注意：不要在Promise外部调用setData设置start_info或end_info
  },
